...
export function groupTransits(list) {
  ...
  return {
    immediate: ranked.filter((t) => !t.background).slice(0, IMMEDIATE_LIMIT),
    background: ranked.filter((t) => t.background).slice(0, BACKGROUND_LIMIT),
...
export function summarise(groups) {
  ...
  if (!immediate.length && !background.length) return null;
  const counts = {};
  ...
  const parts = [];
  parts.push(immediate.length
    ? `${immediate.length} active contact${immediate.length === 1 ? "" : "s"} to your chart right now.`
    : "No fast-moving contacts to your chart right now.");
  if (busiest && busiest[1] > 1) {
    parts.push(`Your natal ${busiest[0]} is receiving the most attention, with ${busiest[1]} contacts.`);
  }
  if (tightest) {
    parts.push(`The closest is ${tightest.transiting} ${tightest.aspect.toLowerCase()} your ${tightest.natal}, ${tightest.orbLabel} from exact.`);
  }
  if (background.length && !immediate.length) {
    parts.push("What is present is slower background movement rather than anything immediate.");
  }
