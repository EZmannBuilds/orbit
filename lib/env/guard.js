...
export function assertStartupSafe(info = resolveEnvironment()) {
  ...
  if ((info.isLocal || info.isTest)
    && (info.databaseTarget === "unknown" || info.databaseTarget === "preview"
      ...
      || info.databaseTarget === "approved-shared-production")) {
    throw new EnvironmentSafetyError(
      [
  ...
  // Production, explicitly approved to share the Orbit database. Announced, not
  // silent: a Production deployment writing to the same rows a Preview writes to
  // is a fact somebody must be able to see in the logs.
  if (info.isProduction && info.databaseTarget === "approved-shared-production") {
    console.warn(
      [
        "",
        `  ⚠  PRODUCTION IS USING THE SHARED ORBIT DATABASE (project ${info.projectRef}).`,
        ...sharedPreviewWarnings().map((line) => `     ${line}`),
        "",
      ].join("\n"),
    );
  } else if (info.isProduction && info.sharedProduction?.requested && !info.sharedProduction.approved) {
    throw new EnvironmentSafetyError(
      [
        "Orbit stopped before startup because production shared-database mode was requested but refused.",
        "",
        `Reason: ${info.sharedProduction.reason}.`,
        "",
        "All of these must be set for a production deployment to share the Orbit database:",
      ...
      { code: "production_shared_not_approved", info },
    );
  }
