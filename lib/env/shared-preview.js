...
import { serviceRoleVerdict } from "./service-role.js";

/** The single accepted value. Anything else — including close misspellings. */
export const SHARED_ORBIT_MODE = "shared-orbit";

/**
 * The two environments that can be approved to share the Orbit database, and
 * the variables each one reads.
 *
 * Preview and Production have SEPARATE variables on purpose. Reusing
 * ORBIT_PREVIEW_PROJECT_REFS for Production would mean approving a Preview
 * silently approved Production too — the blast radius of the two decisions is
 * completely different, and they should have to be made twice.
 */
export const SHARED_MODES = Object.freeze({
  preview: {
    environment: "preview",
    modeVar: "ORBIT_PREVIEW_DATABASE_MODE",
    refsVar: "ORBIT_PREVIEW_PROJECT_REFS",
    target: "approved-shared-preview",
  },
  production: {
    environment: "production",
    modeVar: "ORBIT_PRODUCTION_DATABASE_MODE",
    refsVar: "ORBIT_PRODUCTION_PROJECT_REFS",
    target: "approved-shared-production",
  },
});
...
export function sharedPreviewVerdict(env = process.env, context = {}) {
  return sharedDatabaseVerdict("preview", env, context);
}

/** Production's own approval. Same rules, separate variables, separate decision. */
export function sharedProductionVerdict(env = process.env, context = {}) {
  return sharedDatabaseVerdict("production", env, context);
}

/**
 * @param {"preview"|"production"} which
 */
export function sharedDatabaseVerdict(which, env = process.env, context = {}) {
  const spec = SHARED_MODES[which];
  if (!spec) return { requested: false, approved: false, reason: "unknown environment", mode: null };
  const declared = String(env[spec.modeVar] || "").trim();
  const requested = declared.length > 0;

  const refuse = (reason) => ({ requested, approved: false, reason, mode: declared || null, which });

  if (!requested) return refuse(`no shared-database mode was requested (${spec.modeVar} is unset)`);
  ...
  if (environment !== spec.environment) {
    return refuse(`${spec.modeVar} applies to ${spec.environment} only, not "${environment || "unknown"}"`);
  }
  ...
  if (context.isVercel && String(context.vercelEnv || "").toLowerCase() !== spec.environment) {
    return refuse(`Vercel reports "${context.vercelEnv || "unknown"}", not ${spec.environment}`);
  }
  ...
  const allowed = String(env[spec.refsVar] || "")
    .split(",").map((s) => s.trim()).filter(Boolean);
  if (!allowed.length) return refuse(`${spec.refsVar} is not set`);
  if (!allowed.includes(urlRef)) {
    return refuse(`the project in SUPABASE_URL is not listed in ${spec.refsVar}`);
  }
  ...
  // A service-role key bypasses row-level security for every table.
  //
  // In a PREVIEW that is absolute and always will be: a Preview is where
  // unreviewed code runs, and it has no need for the key — it authenticates
  // users with the anon key and RLS like everyone else.
  //
  // In PRODUCTION it is conditional, because account deletion genuinely
  // requires the Supabase Admin API and there is no other way to close an
  // account. The condition is narrow and lives in lib/env/service-role.js:
  // a named purpose, a separately pinned project reference, production only,
  // and Vercel agreeing. Anything short of all of that refuses here exactly as
  // before.
  if (env.SUPABASE_SERVICE_ROLE_KEY) {
    const allowance = which === "production"
      ? serviceRoleVerdict("account-deletion", env, context)
      : { authorized: false, reason: "preview never holds a service-role key" };
    if (!allowance.authorized) {
      return refuse(
        `a service-role key must not be configured for a shared-database ${spec.environment}`
        + ` (${allowance.reason})`,
      );
    }
  }
  if (env.SUPABASE_DB_PASSWORD) {
    return refuse(`a database password must not be configured for a shared-database ${spec.environment}`);
  }
  ...
  for (const [name, value] of Object.entries({
    ...
    if (String(value || "").trim()) {
      return refuse(`${name} must not be set for a shared-database ${spec.environment}`);
    }
  }
