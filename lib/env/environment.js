...
import { sharedPreviewVerdict, sharedProductionVerdict } from "./shared-preview.js";
...
export function resolveEnvironment({ env = null, overrides = null, loadEnvFiles = true } = {}) {
  ...
  const sharedPreview = sharedPreviewVerdict(effective, sharedContext);
  // Production has its own approval, reading its own variables. Approving a
  // Preview must never silently approve Production: the blast radius differs
  // completely, so the decision has to be made twice.
  const sharedProduction = sharedProductionVerdict(effective, sharedContext);
  const sharedDatabase = sharedProduction.approved ? sharedProduction : sharedPreview;
  ...
  return {
    ...
    databaseTarget: sharedDatabase.approved ? sharedDatabase.target : target,
