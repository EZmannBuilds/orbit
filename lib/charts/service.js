...
// Preference: the primary "My Chart" → most recently updated → most recently
// created → first. Never returns a chart the owner doesn't have.
...
export function createChartService(store) {
  ...
  async function resolveActive(ownerId, profiles) {
    ...
    // Persist best-effort: a failed write must not break the read path — the
    // caller still gets a usable active chart for this request.
    try { await store.setActiveId(ownerId, fallback.id); } catch { /* non-fatal */ }
    return fallback;
  }
