...
export async function handleChartsRoute(method, route, query, body, auth = null, raw = {}) {
  ...
  try {
    ...
    // ── Chart avatars ───────────────────────────────────────────────────
    //
    // Ownership is established by svc.get BEFORE any storage call, so a chart
    // id belonging to someone else fails here exactly as it does everywhere
    // else — the Storage policies are the second line, not the first.
    //
    // The raw object path is built from ids the server already holds and is
    // never returned; responses carry publicIdentity only.
    if (action === "avatar") {
      const { chart } = await svc.get(owner, id);          // throws not_found for others
      const store = createAvatarStore(auth);
      const path = avatarObjectPath(owner, chart.id);

      if (method === "GET") {
        if (!chart.avatar_storage_path) return err(404, "No picture for this chart", { code: "avatar_not_found" });
        // A version-keyed validator: replacing the image changes the tag, so a
        // stale picture cannot survive in a cache.
        const headers = avatarCacheHeaders(chart.avatar_version, "image/webp");
        ...
        return { status: 200, headers, body: got.bytes, binary: true };
      }
      ...
      if (method === "POST") {
        ...
        // Metadata moves only after the bytes are safely stored. If THIS fails
        // the object exists but the row still points at the old version, which
        // is recoverable by uploading again; the reverse order would leave a
        // row pointing at bytes that were never written.
        const updated = await svc.update(owner, id, {
          avatar_storage_path: path,
          avatar_version: Number(chart.avatar_version || 0) + 1,
          avatar_updated_at: new Date().toISOString(),
        });
        return ok({ identity: publicIdentity(updated.chart || updated) });
      }
