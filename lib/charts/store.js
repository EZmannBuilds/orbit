...
export function createSupabaseChartStore(auth = null) {
  return {
  async listProfiles(ownerId) {
    const r = await req(auth, "GET", `birth_profiles?owner_id=eq.${ownerId}&order=created_at.asc`);
    return r.ok ? r.data : [];
  },
  async getProfile(ownerId, id) {
    const r = await req(auth, "GET", `birth_profiles?owner_id=eq.${ownerId}&id=eq.${id}&limit=1`);
    return r.ok && r.data?.length ? r.data[0] : null;
  },
  async countProfiles(ownerId) {
    const r = await req(auth, "GET", `birth_profiles?owner_id=eq.${ownerId}&select=id`);
    return r.ok ? r.data.length : 0;
  },
  async insertProfile(row) {
    const r = await req(auth, "POST", "birth_profiles", row, { prefer: "return=representation" });
    if (!r.ok) throw new Error(r.error || r.reason || "insert_failed");
    return Array.isArray(r.data) ? r.data[0] : r.data;
  },
  async updateProfile(ownerId, id, patch) {
    const r = await req(auth, "PATCH", `birth_profiles?owner_id=eq.${ownerId}&id=eq.${id}`, patch, { prefer: "return=representation" });
    if (!r.ok) throw new Error(r.error || r.reason || "update_failed");
    return Array.isArray(r.data) ? r.data[0] : r.data;
  },
  // Conditional update: the patch lands only where every guard column still
  // holds the value the caller read. Zero matched rows returns null rather
  // than throwing — "the row moved" is an answer the service turns into a
  // stale-write refusal, not a transport failure. This is what makes the
  // avatar version an enforced optimistic lock instead of a read-then-hope.
  async updateProfileGuarded(ownerId, id, guards, patch) {
    const filters = Object.entries(guards || {})
      .map(([column, value]) => `&${encodeURIComponent(column)}=eq.${encodeURIComponent(value)}`)
      .join("");
    const r = await req(auth, "PATCH", `birth_profiles?owner_id=eq.${ownerId}&id=eq.${id}${filters}`, patch, { prefer: "return=representation" });
    if (!r.ok) throw new Error(r.error || r.reason || "update_failed");
    return Array.isArray(r.data) && r.data.length ? r.data[0] : null;
  },
  async activateProfile(ownerId, id) {
    const r = await req(auth, "POST", "rpc/activate_birth_profile", { p_birth_profile_id: id });
    if (!r.ok) throw new Error(r.error || r.reason || "activate_failed");
    return Array.isArray(r.data) ? r.data[0] : r.data;
  },
  async deleteProfile(ownerId, id) {
    const r = await req(auth, "DELETE", `birth_profiles?owner_id=eq.${ownerId}&id=eq.${id}`);
    if (!r.ok) throw new Error(r.error || r.reason || "delete_failed");
    return true;
  },
  async getActiveId(ownerId) {
    const r = await req(auth, "GET", `profiles?user_id=eq.${ownerId}&select=active_birth_profile_id&limit=1`);
  ...
  async setActiveId(ownerId, id) {
    ...
    const r = await req(auth, "POST", "profiles?on_conflict=user_id",
      { user_id: ownerId, active_birth_profile_id: id },
      { prefer: "resolution=merge-duplicates,return=minimal" });
    if (!r.ok) throw new Error(r.error || r.reason || "set_active_failed");
    return true;
  },
  async upsertProfileNames(ownerId, firstName, lastName) {
    const r = await req(auth, "POST", "profiles?on_conflict=user_id",
      { user_id: ownerId, first_name: firstName, last_name: lastName },
      { prefer: "resolution=merge-duplicates,return=minimal" });
    if (!r.ok) throw new Error(r.error || r.reason || "profile_names_failed");
    return true;
  },
  ...
  async insertCalculation(row) {
    const r = await req(auth, "POST", "chart_calculations", row, { prefer: "return=representation" });
...
export const supabaseChartStore = createSupabaseChartStore();
