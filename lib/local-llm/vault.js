...
export function normalizeVaultRelPath(relPath) {
  const cleaned = String(relPath || "").replaceAll("\\", "/").replace(/^\/+/, "");
  if (!cleaned || cleaned.includes("\0")) throw new Error("Path is empty or invalid.");
  if (cleaned.startsWith("/") || cleaned.match(/^[A-Za-z]:\//)) throw new Error("Absolute paths are not allowed.");
...
export function validateManagedNoteContent(content, options = {}) {
  ...
  for (const key of ["id", "title", "type", "status", "created_at", "updated_at"]) {
...
export function createProposal({ operation, path: notePath, reason = "", content = "", appendContent = "", model = "deterministic-fallback", sources = [], vaultPath = localLlmConfig().vaultPath }) {
  ...
  const validation = validateManagedNoteContent(proposed, exists ? beforeParsed : null);
  const proposal = {
    id: randomUUID(),
    operation: op,
    path: relPath,
