  ...
  return async function orbitRequestHandler(req, res) {
    ...
    try {
      ...
      if (route.startsWith("/api/compatibility/")) {
        const auth = await requireAuth(req, res, env);
        if (!auth) return;
        const handled = await handleCompatibilityRoute(req.method, route, url.searchParams, {}, authContext(auth));
      ...
      if (route === "/api/charts" || route.startsWith("/api/charts/")
        ...
        const handled = await handleChartsRoute(req.method, route, url.searchParams, body, authContext(auth), raw);
        if (handled) {
          const cookie = auth?.setCookie ? { "Set-Cookie": auth.setCookie } : {};
