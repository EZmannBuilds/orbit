...
// ── platform capability ──────────────────────────────────────────────────────
// The bundled `swetest` is a compiled native binary, so it only runs on the
// platform it was built for. The copy in this repository is Mach-O arm64 (Apple
// Silicon); a Linux x86-64 host — which is what a Vercel function is — cannot
// execute it and fails with ENOEXEC.
//
// Without this check that surfaces as an opaque 500 on every chart, sky,
// fortune, and Ask Orbit request. With it, the failure names itself once, which
// is what turns "the deploy is broken" into a specific, fixable finding.
//
// This is a diagnostic, not a fallback: there is no second ephemeris to fall
// back to, and inventing positions would be worse than failing. See
// scripts/deploy-check.js, which reports this before a deploy rather than after.
...
export function ephemerisCapability() {
  if (capability) return capability;
  if (!existsSync(SWETEST)) {
    capability = { ok: false, code: "binary_missing", detail: "The swetest binary is not present in lib/astro/bin." };
    return capability;
  }
  if (!existsSync(EPHE_DIR)) {
    capability = { ok: false, code: "ephemeris_data_missing", detail: "The .se1 ephemeris data files are not present in lib/astro/ephe." };
    return capability;
  }
  try {
    // -h is a trivial, side-effect-free invocation: it proves the host can
    // execute this binary at all without computing anything.
    execFileSync(SWETEST, ["-h"], { encoding: "utf8", timeout: 5000, stdio: "pipe" });
    capability = { ok: true, code: "ok", detail: `${EPHEMERIS_VERSION} is executable on ${process.platform}/${process.arch}.` };
  } catch (error) {
    const code = error?.code === "ENOEXEC" ? "wrong_architecture" : "not_executable";
    capability = {
      ok: false,
      code,
      detail: code === "wrong_architecture"
        ? `The bundled swetest binary cannot run on ${process.platform}/${process.arch}. It is built for a different platform.`
        : `The bundled swetest binary could not be executed on ${process.platform}/${process.arch}.`,
    };
  }
  return capability;
}
...
function run(args) {
  try {
    ...
    if (error?.code === "ENOEXEC" || error?.code === "EACCES" || error?.code === "ENOENT") {
      const cap = ephemerisCapability();
      throw new EphemerisUnavailableError(
        `Orbit's astronomy engine is unavailable in this environment. ${cap.detail}`,
        { code: cap.code },
      );
    }
    throw error;
