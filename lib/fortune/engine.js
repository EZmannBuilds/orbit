...
function buildFactors(sky, chart, transits) {
  ...
  f.push({
    ...
    balanced: `Sun in ${sky.zodiac_season}`,
  ...
  f.push({
    ...
    balanced: `${sky.moon.phase_name} Moon in ${sky.moon.sign}, ${Math.round(sky.moon.illumination_percent)}% illuminated`,
  ...
  for (const r of sky.retrogrades) {
    f.push({ type: "retrograde", simple: `${r} is retrograde (a review-and-slow-down signal)`, balanced: `${r} retrograde`, advanced: `${r} retrograde` });
  }
  for (const t of transits.slice(0, 3)) {
    f.push({
      type: "transit",
      simple: plainTransit(t),
      balanced: `${t.transiting} ${t.aspect} your natal ${t.natal}`,
...
// Render the factor list at a given detail level (Simple | Balanced | Advanced).
export function factorsForLevel(factors, level) {
  const key = level === "Advanced" ? "advanced" : level === "Balanced" ? "balanced" : "simple";
  return factors.map((f) => f[key]);
}
