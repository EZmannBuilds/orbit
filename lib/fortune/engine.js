...
export function composeFortune({ chart, sky, localDate, timezoneName, chartId, chartInputHash }) {
  ...
  if (sky.retrogrades.includes("Mercury")) {
    watch = "Messages and plans may need a second look — confirm details before you lock them in";
  } else {
    const hard = transits.find((t) => t.hard);
    watch = hard
      ? `Some friction is in play today — ${pick(seed, "watch", WATCHOUT_GENERAL)}`
