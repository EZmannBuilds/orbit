...
export function createSupabaseFortuneStore(auth = null) {
  return {
  ...
  async insertFortune(row) {
    const r = await req(auth, "POST", "daily_fortunes", row, { prefer: "return=representation,resolution=merge-duplicates" });
  ...
  async listHistory(ownerId, { birthProfileId = null, limit = 30 } = {}) {
    ...
    const r = await req(auth, "GET", q);
    return r.ok ? r.data : [];
  },
  async getDetailLevel(ownerId) {
    const r = await req(auth, "GET", `profiles?user_id=eq.${ownerId}&select=astrology_detail_level&limit=1`);
  ...
  async getCurrentTimezone(ownerId) {
    const r = await req(auth, "GET",
      `profiles?user_id=eq.${ownerId}&select=current_timezone_name,current_timezone_source,current_timezone_updated_at&limit=1`);
    return r.ok && r.data?.length ? r.data[0] : null;
  },
  async setCurrentTimezone(ownerId, { name, source }) {
    const r = await req(auth, "POST", "profiles?on_conflict=user_id",
      { user_id: ownerId, current_timezone_name: name, current_timezone_source: source, current_timezone_updated_at: new Date().toISOString() },
      { prefer: "resolution=merge-duplicates,return=minimal" });
    if (!r.ok) throw new Error(r.error || r.reason || "set_current_timezone_failed");
    return true;
  },
};
}

export const supabaseFortuneStore = createSupabaseFortuneStore();
