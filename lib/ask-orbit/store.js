...
export function askStorageMode(auth = null) {
  return usesPersistentStore(auth) ? "persistent" : "session";
}

export function askStoreFor(auth = null) {
  return usesPersistentStore(auth) ? createSupabaseAskStore(auth) : memoryAskStore;
}
