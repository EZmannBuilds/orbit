...
import {
  currentOwnerId, isConfigured, askStorageMode, askStoreFor,
} from "./store.js";
import { cachedHealth } from "../local-llm/axis-chat.js";
...
export async function handleAskRoute(method, route, query, body, auth = null) {
  ...
  if (route === "/api/ask/suggestions" && method === "GET") {
    const chartSvc = createChartService(auth ? createSupabaseChartStore(auth) : supabaseChartStore);
    let active = null;
    // Distinguish "genuinely zero charts" (active === null) from "the chart
    // request failed" so the client never tells a returning user to add a chart
    // they already have.
    let chartStatus = "ok";
    try { active = await chartSvc.getActive(owner); } catch { chartStatus = "error"; }
    const sky = cachedSky();
    const suggestions = suggestedQuestions({ active, sky });

    // Storage state: is history actually durable right now, and can we read it?
    const mode = askStorageMode(auth);
    let historyReadable = true;
    try { await svc.listConversations(owner, { limit: 1 }); }
    catch { historyReadable = false; }

    // Optional language enhancement (shared 5s health cache — no extra load).
    let enhanced = false;
    if (process.env.ORBIT_ASK_USE_MODEL !== "false") {
      try {
        const h = await cachedHealth(createLocalLLMProvider());
        enhanced = !!(h?.reachable && (h.model_available ?? h.installed_model));
    ...
    return ok({
      ...
      storage: {
        ...
        history_available: historyReadable,
      },
      language_enhancement: { available: enhanced },
    });
  }
  ...
  if (route === "/api/ask" && method === "POST") {
    ...
    try {
      ...
      return ok({
        ...
        // Never imply a save that didn't happen.
        persisted: result.persisted !== false,
        storage_note: result.persisted === false
          ? "This answer couldn't be saved to your history."
          : null,
